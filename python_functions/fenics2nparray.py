...
def fenics2nparray(data, boundary_value, r_hole, x, y, hole_coor):
    ...
    if len(hole_coor) == 1:
        r0 = np.sqrt((X-hole_coor[0][0])**2 + (Y-hole_coor[0][1])**2)
        r1 = np.ones([Nx,Ny])*1e3
        r2 = np.ones([Nx,Ny])*1e3
    elif len(hole_coor) == 2:
        r0 = np.sqrt((X-hole_coor[0][0])**2 + (Y-hole_coor[0][1])**2)
        r1 = np.sqrt((X-hole_coor[1][0])**2 + (Y-hole_coor[1][1])**2)
        r2 = np.ones([Nx,Ny])*1e3
    elif len(hole_coor) == 3:
        r0 = np.sqrt((X-hole_coor[0][0])**2 + (Y-hole_coor[0][1])**2)
        r1 = np.sqrt((X-hole_coor[1][0])**2 + (Y-hole_coor[1][1])**2)
        r2 = np.sqrt((X-hole_coor[2][0])**2 + (Y-hole_coor[2][1])**2)
    else:
